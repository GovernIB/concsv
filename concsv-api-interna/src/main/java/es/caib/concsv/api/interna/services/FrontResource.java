...
public class FrontResource {
	...
	public Response decodeHashPrintable(
		...
		return executarIEnregistrar(EnviamentTipus.IMPRIMIBLE.name(), () -> {
	...
	public Response decodeHashEniDoc(
		@Parameter(description = "Codi CSV del document") @PathParam("csv") String csv) {
		return executarIEnregistrar(EnviamentTipus.ENI.name(), () -> {
			DocumentInfo documentInfo = hashService.checkHash(csv);
			if (documentInfo == null) {
				return Response.status(Response.Status.NO_CONTENT).build();
			}
	...
	public Response decodeHashMetadata(
		@Parameter(description = "Codi CSV del document") @PathParam("csv") String csv) {
		return executarIEnregistrar(EnviamentTipus.METADATOS.name(), () -> {
			DocumentInfo documentInfo = hashService.checkHash(csv);
			if (documentInfo == null) {
				return Response.status(Response.Status.NO_CONTENT).build();
			}
	...
	@GET
	@Path("/v1/salut/info")
	@Operation(summary = "(Integració amb Comanda) Retorna informació de l'aplicació", security = @SecurityRequirement(name = "basic-auth"))
	public Response appInfo(@Context HttpServletRequest request) throws IOException, GenericServiceException {
		return Response
			.ok(comandaService.appInfo(getManifestProperties(), getServerRoot(request)),
				MediaType.APPLICATION_JSON_TYPE)
			.build();
	}
	...
	@Operation(summary = "(Integració amb Comanda) Recupera les dades de l'últim dia registrat", security = @SecurityRequirement(name = "basic-auth"))
	public RegistresEstadistics estadistiques() {
		return comandaService.consultaUltimesEstadistiques();
	}

	@GET
	@Path("/v1/estadistiques/info")
	@Operation(summary = "(Integració amb Comanda) Retorna les dimensions disponibles", security = @SecurityRequirement(name = "basic-auth"))
	public EstadistiquesInfo statsInfo() {
		List<DimensioDesc> dimensions = comandaService.getDimensions();
		List<IndicadorDesc> indicadors = comandaService.getIndicadors();
		return new EstadistiquesInfo().codi("CSV").dimensions(dimensions).indicadors(indicadors);
	}

	@GET
	@Path("/v1/estadistiques/{dies}")
	@Operation(summary = "(Integració amb Comanda) Recupera les dades dels últims dies registrats", security = @SecurityRequirement(name = "basic-auth"))
	public List<RegistresEstadistics> estadistiques(
		...
		for (int i = 0; i < dies; i++) {
			result.add(comandaService.consultaEstadistiques(data));
	...
	@Operation(summary = "(Integració amb Comanda) Recupera les dades del dia indicat", security = @SecurityRequirement(name = "basic-auth"))
	public RegistresEstadistics estadistiques(
		...
		return comandaService.consultaEstadistiques(date);
	}

	@GET
	@Path("/v1/estadistiques/from/{dataInici}/to/{dataFi}")
	@Operation(summary = "(Integració amb Comanda) Recupera les dades entre les dues dades sol·licitades", security = @SecurityRequirement(name = "basic-auth"))
	public List<RegistresEstadistics> estadistiques(
		...
		return comandaService.consultaEstadistiques(startDate, endDate);
	}

	@GET
	@Path("/v1/logs")
	@Operation(summary = "(Integració amb Comanda) Retorna el llistat de logs disponibles", security = @SecurityRequirement(name = "basic-auth"))
	public Response llistarFitxers() throws GenericServiceException {
		return Response.ok(comandaService.llistarFitxers(), MediaType.APPLICATION_JSON_TYPE).build();
	}

	@GET
	@Path("/v1/logs/{nomFitxer}")
	@Operation(summary = "(Integració amb Comanda) Retorna el fitxer sol·licitat", security = @SecurityRequirement(name = "basic-auth"))
	public Response getFitxerByNom(
		@Parameter(description = "Nom fitxer a recuperar") @PathParam("nomFitxer") String nomFitxer)
		throws GenericServiceException {
		return Response.ok(comandaService.getFitxerByNom(nomFitxer), MediaType.APPLICATION_JSON_TYPE).build();
	}

	@GET
	@Path("/v1/logs/{nomFitxer}/linies/{nLinies}")
	@Operation(summary = "(Integració amb Comanda) Retorna el fitxer sol·licitat", security = @SecurityRequirement(name = "basic-auth"))
	public Response getFitxerByNom(
		@Parameter(description = "Nom fitxer a recuperar") @PathParam("nomFitxer") String nomFitxer,
		@Parameter(description = "Nombre de linies a recuperar (Sempre retornarà un mínim de línies i té un valor màxim de línies)") @PathParam("nLinies") Long nLinies)
		throws GenericServiceException {
		return Response.ok(comandaService.llegitUltimesLinies(nomFitxer, nLinies), MediaType.APPLICATION_JSON_TYPE).build();
	}
	 ...
	 * en cas d’excepció.
	 */
	private Response executarIEnregistrar(@NotNull String key, Callable<Response> accion) {
		...
		try {
			...
			if (!(ex instanceof GenericServiceException) || ((GenericServiceException) ex).isLoggable()) {
				log.error("Error executant l'acció {}: {}", fullKey, ex.getMessage(), ex);
			} else {
				log.error("Error executant l'acció {}: {}", fullKey, ex.getMessage());
			}
			...
			try {
				comandaService.registrarPeticio(fullKey, resultType, duration);
			} catch (Exception e) {
