...
public class SubsistemesHelper {
	...
	private List<SubsistemaSalut> getSubsistemesSalut() {
		...
		for (SubsistemesEnum s : SubsistemesEnum.values()) {
			Metrics m = metrics.computeIfAbsent(s, k -> new Metrics());
			final int latencia = (m.timerOkLocal != null && m.timerOkLocal.count() > 0) ? (int) m.timerOkLocal.mean(TimeUnit.MILLISECONDS) : 0;
			...
			final int totalTempsMig = (m.timerOkGlobal != null && m.timerOkGlobal.count() > 0) ? (int) m.timerOkGlobal.mean(TimeUnit.MILLISECONDS) : 0;
