...
/**
 * Autenticació de la interfície REACT amb la sessió de servidor, com fan RIPEA (`RipeaAuthProvider`)
 * i DISTRIBUCIO (`DistribucioAuthProvider`).
 *
 * El navegador no gestiona cap token: les crides a l'API viatgen amb la galeta de la sessió HTTP
 * (adaptador Keycloak de JBoss en mode EAR, `oauth2Login` de Spring en mode Spring Boot), i la
 * renovació del token de Keycloak, si n'hi ha, la fa el servidor dins d'aquesta sessió. Amb un token
 * per pestanya (`OidcAuthProvider` de base-react), totes les pestanyes compartien la mateixa sessió
 * de client de Keycloak i la renovació d'una invalidava la de les altres ("Stale token"): la que
 * fallava es recarregava sencera cada pocs minuts.
 *
 ...
 * arribaria mai a caducar. Es consulta en tornar a la pestanya i amb l'activitat de l'usuari (com a
 * molt un cop per minut).
 *
...
export const useSessioUsuari = (): SessioUsuariContextType => {
    ...
    if (context === undefined) {
        throw new Error('useSessioUsuari must be used within a ConcsvAuthProvider');
    }
    return context;
};
...
/** Temps mínim entre dues comprovacions provocades en tornar a la pestanya. */
 ...
 */
const INICI_SESSIO_KEY = 'concsv.sessioCaducada.intent';
...
type ConcsvAuthProviderProps = React.PropsWithChildren & {
    /** URL de l'API REST, d'on es dedueix l'arrel de l'aplicació al servidor. */
    apiUrl: string;
};

export const ConcsvAuthProvider: React.FC<ConcsvAuthProviderProps> = ({ apiUrl, children }) => {
    ...
    const authContext = React.useMemo(() => {
        ...
        return {
            ...
            signIn: undefined,
            // Tancament de sessió del servidor (veure LogoutController i WebSecurityConfig). NO
            // facis un fetch del logout: ha d'acabar redirigint a Keycloak, i cal una navegació real perquè el
...
export default ConcsvAuthProvider;
